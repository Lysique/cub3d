...
/*   Updated: 2022/02/12 13:24:10 by tamighi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_f_c(t_cub *cub, t_ray *r, int y)
{
	...
	while (x < WIN_W - (OPTIMISATION - 1))
	{
		...
		r->tex_x = (cub->textures[C].w * (r->floor_x - r->map_x));
		r->tex_y = (cub->textures[C].h * (r->floor_y - r->map_y));
		color = get_texture_color(cub->textures[C], (int)r->tex_y, (int)r->tex_x);
		optimisation_pixel_put(cub->display, y, x, color);
		r->tex_x = (cub->textures[F].w * (r->floor_x - r->map_x));
		r->tex_y = (cub->textures[F].h * (r->floor_y - r->map_y));
		color = get_texture_color(cub->textures[F], (int)r->tex_y, (int)r->tex_x);
		optimisation_pixel_put(cub->display, WIN_H - y - 1, x, color);
...
void	f_c_casting_init(t_cub *cub, t_ray *r, int y)
{
	...
	r->tex_stepx = (r->row_dist * (r->dir2_x - r->dir_x) / WIN_W) * OPTIMISATION;
	r->tex_stepy = (r->row_dist * (r->dir2_y - r->dir_y) / WIN_W) * OPTIMISATION;
